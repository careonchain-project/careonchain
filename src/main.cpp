...
#if defined(NDEBUG)
# error "Careonchain cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcd5bf31615df463302d58dc9aa52b594869f84be25d6bb84d37d3a1f4916354d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Careonchain: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Careonchain Signed Message:\n";
...
{
    // Careonchain: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Careonchain: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Careonchain: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Careonchain: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Careonchain: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Careonchain: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa9d4eb5c489030e2e0aeca6e9e50d30df356c46c05a89e3b5ccb6388bef6dcd7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CAREONchain will ROCK";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0459482ac90238ecd63e6933a742542e9125cd2672fa8d8ec0aca541b40552661db63444bcf01cc95bb8bf4e8fd79dea930cea1cc7bd276f5ccda6eb968b29a2e7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1531470404;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084615512;

        if (fTestNet)
        {
            block.nTime    = 1531470389;
            block.nNonce   = 385302527;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb98d95807cfc1cd0916b61e7db6e7d1f3f29145a116f43627f545d4e117e852d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Careonchain: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CareonchainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CareonchainMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CareonchainMiner(CWallet *pwallet)
{
    printf("CareonchainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("careonchain-miner");
    ...
    try { loop {
        ...
        printf("Running CareonchainMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CareonchainMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CareonchainMiner, pwallet));
}
